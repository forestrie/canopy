...
export function validateOperationId(id: string | undefined): Response | null {
  ...
  return (isOperationFormat || isUuid) ? null : ClientErrors.badRequest('Invalid operationId format');
}
...
export function validateScrapiRequest(
  ...
  if (request.method === 'POST' || request.method === 'PUT') {
    ...
    if (options.contentTypes) {
      const matches = options.contentTypes.some(ct => contentType.toLowerCase().includes(ct.toLowerCase()));
      if (!matches) {
