...
describe('Canopy API', () => {
  ...
  it('should return SCITT configuration', async () => {
    const request = new Request('http://localhost/.well-known/scitt-configuration');
  ...
  it('should register a COSE statement', async () => {
    ...
    expect(response.headers.get('Location')).toMatch(/http:\/\/localhost\/logs\/logid-1\/entries\/\d{8}\/[a-f0-9]{32}$/);
  });
