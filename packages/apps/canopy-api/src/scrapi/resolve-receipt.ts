 ...
 *
 * Success `Content-Type` (FOR-559): the receipt media type
 * draft-ietf-scitt-scrapi-05 §6.3 registers, `application/scitt.receipt+cose`,
 * for every request regardless of `Accept`. The 404/problem path is
 * unaffected.
 */
...
export async function resolveReceipt(
  ...
  try {
    ...
    return cborResponse(assembled, 200, {
      "content-type": CBOR_CONTENT_TYPES.SCITT_RECEIPT_COSE,
      ...NO_STORE_HEADERS,
    });
  } catch (error) {
