...
export function createReceiptVerifyKeyResolver(config: {
  ...
  return async (ownerLogIdLowerHex32: string) => {
    console.log("receipt-verify-key-resolver: resolving key for", {
    ...
    if (!p) {
      p = (async (): Promise<ParsedVerifyKey> => {
        ...
        try {
          console.log(
            "receipt-verify-key-resolver: trying direct custody key fetch",
          );
          ...
          console.log("receipt-verify-key-resolver: got custody key", {
            alg,
            pemLen: publicKeyPem.length,
          });
          ...
          console.log("receipt-verify-key-resolver: imported key", {
            ...
            curve:
              key instanceof CryptoKey ? "(CryptoKey)" : (key as any).curve,
          });
          return key;
        } catch (e) {
          console.log(
            "receipt-verify-key-resolver: direct custody fetch failed",
