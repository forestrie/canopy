 ...
 *
 * Supported value types (sufficient for COSE headers / SCITT receipts):
 * unsigned + negative integers (`number` | `bigint`), byte strings
 * (`Uint8Array`), text strings (`string`), arrays, and integer/text-keyed maps
 * (`Map`). Any other type throws rather than emitting silently-wrong bytes.
 */
 ...
 *
 * @param value - Integer, byte string, text string, array, or Map
 * @returns Deterministic CBOR bytes (definite lengths, sorted map keys, no tags)
 * @throws On unsupported value types (float, boolean, null, plain object, …)
 */
export function encodeCborDeterministic(value: unknown): Uint8Array {
  ...
  throw new Error(
    `encodeCborDeterministic: unsupported value type ${
