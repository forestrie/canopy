...
function normalize(v: unknown): unknown {
  if (v instanceof Uint8Array || Buffer.isBuffer(v)) return hex(new Uint8Array(v as Uint8Array));
...
const CORPUS: unknown[] = [
  ...
  [1, 2, 3],
  [new Uint8Array([1]), new Map([[4, new Uint8Array([9])]]), null, new Uint8Array([2])],
  ...
  new Map<number, unknown>([
    ...
    [
      396,
      new Map<number, unknown>([
        [-1, [new Map<number, unknown>([[1, 0], [2, [new Uint8Array(32).fill(3)]]])]],
      ]),
    ],
  ]),
...
describe("@forestrie/encoding CBOR codec vs reference `cbor`", () => {
  ...
  it("decodes reference-`cbor`-canonical bytes identically", () => {
    ...
    const value = new Map<number, unknown>([[1, -7], [4, Buffer.from([1, 2, 3])]]);
  ...
  it("round-trips a COSE tag(18) via CborTag / unwrap", () => {
    const inner = encodeCborDeterministic([new Uint8Array([1]), new Map(), null, new Uint8Array([2])]);
  ...
  it("§4.2 map key ordering is bytewise (canonical)", () => {
    const bytes = encodeCborDeterministic(new Map([[10, 1], [2, 2], [-1, 3], [100, 4]]));
  ...
  it("rejects duplicate map keys (§4.2 unique-keys)", () => {
    expect(() =>
      encodeCborDeterministic(new Map<unknown, unknown>([[1, "a"], [1n, "b"]])),
    ).toThrow(/duplicate/);
  });
  ...
  it("throws on non-integer numbers and deep nesting / indefinite input", () => {
    ...
    expect(() => decodeCborDeterministic(new Uint8Array([0x9f, 0xff]))).toThrow(/indefinite/);
    expect(() => decodeCborDeterministic(new Uint8Array([0x00, 0x00]))).toThrow(/trailing/);
  });
});
