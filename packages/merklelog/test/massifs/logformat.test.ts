...
import { peakStackEnd, massifLogEntries, LogFormat } from "../../src/massifs/logformat.js";
...
describe("logformat", () => {
  describe("peakStackEnd", () => {
    it("should calculate peak stack end for massif height 0", () => {
      ...
      const expected = BigInt(LogFormat.StartHeaderSize) + BigInt(LogFormat.IndexHeaderBytes) + BigInt(LogFormat.TrieEntryBytes * 1) + BigInt(LogFormat.MaxMmrHeight * LogFormat.ValueBytes);
    ...
    it("should calculate peak stack end for massif height 1", () => {
      ...
      const expected = BigInt(LogFormat.StartHeaderSize) + BigInt(LogFormat.IndexHeaderBytes) + BigInt(LogFormat.TrieEntryBytes * 2) + BigInt(LogFormat.MaxMmrHeight * LogFormat.ValueBytes);
      expect(result).toBe(expected);
    });
    ...
    it("should calculate peak stack end for massif height 3", () => {
      ...
      const expected = BigInt(LogFormat.StartHeaderSize) + BigInt(LogFormat.IndexHeaderBytes) + BigInt(LogFormat.TrieEntryBytes * 8) + BigInt(LogFormat.MaxMmrHeight * LogFormat.ValueBytes);
      expect(result).toBe(expected);
    });
