...
- `canopy-api` uses a test-specific wrangler config (`wrangler.test.jsonc`) that sets **`NODE_ENV` to `test`** and omits cross-worker DO bindings and Custodian bootstrap vars. **Vitest pool workers only:** `NODE_ENV === "test"` skips the bootstrap-trio deployment check (`ROOT_LOG_ID`, `CUSTODIAN_URL`, `CUSTODIAN_BOOTSTRAP_APP_TOKEN`); real workers must define all three or every route returns **503**. Playwright e2e uses dev/prod-like `NODE_ENV`, not `test`.
- When running `pnpm --filter @canopy/api dev` alone, endpoints that call cross-worker Durable Objects (`POST /register/grants`, `POST /register/entries`) return 500 because the forestrie-ingress worker is not running. Start both workers for full integration: `pnpm --filter @canopy/api dev` and `pnpm --filter @canopy/forestrie-ingress dev` in separate terminals.
...
- **Bootstrap grant Playwright (`packages/tests/canopy-api`, `--project=dev`)**: Tests that use the fixed **`DEFAULT_ROOT_LOG_ID`** assume that root log is still on the **bootstrap** branch (no MMRS / uninitialized in storage). After Ranger sequences real grants for that log on api-dev, **`POST /register/grants`** takes the **receipt-based inclusion** path and returns **403** (“header **396**” / SCITT receipt) for a Custodian-only mint — that is **expected**, not a queue outage. The **random `logId`** test (including **second** `POST /register/grants` with a completed Forestrie-Grant after resolve-receipt) exercises bootstrap + sequencing + receipt auth; run that case for healthy queue verification, or use a fresh root log ID in dev MMRS.
