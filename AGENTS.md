...
- **Bootstrap grant Playwright (`packages/tests/canopy-api`, `--project=dev`)**: Tests that use the fixed **`DEFAULT_ROOT_LOG_ID`** assume that root log is still on the **bootstrap** branch (no MMRS / uninitialized in storage). After Ranger sequences real grants for that log on api-dev, `POST /logs/{logId}/grants` takes the **receipt-based inclusion** path and returns **403** (“header **396**” / SCITT receipt) for a Custodian-only mint — that is **expected**, not a queue outage. The **random `logId`** test exercises bootstrap + sequencing end-to-end; run that case for healthy queue verification, or use a fresh root log ID in dev MMRS.
