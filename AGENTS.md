...
- **Bootstrap grant Playwright (`packages/tests/canopy-api`, `--project=dev`)**: Mint is **runner-side** (Custodian `:bootstrap` + **`CURATOR_ADMIN_TOKEN`** to `POST /api/forest/{log-id}/genesis`), then **`POST /register/{log-id}/grants`**. Tests that use the fixed **`DEFAULT_ROOT_LOG_ID`** assume that root log is still on the **bootstrap** branch (no MMRS / uninitialized in storage). After Ranger sequences real grants for that log on api-dev, a bare Custodian-only mint returns **403** on the **receipt-based inclusion** path — that is **expected**, not a queue outage. The **random `logId`** test (including **second** `POST /register/{log-id}/grants` with a completed Forestrie-Grant after resolve-receipt) exercises bootstrap + sequencing + receipt auth; run that case for healthy queue verification, or use a fresh root log ID in dev MMRS.
