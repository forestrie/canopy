...
import {
  encodeCoseSign1WithKid,
  generateUniquePayloadBytes,
} from "../lib/cose.js";
import {
  postAndMaybeWaitWithGrant,
  postLatency,
  postErrors,
  e2eLatency,
  e2eSuccessCount,
...
// Load pre-created grant pool (generated by workflow or perf/scripts/generate-grant-pool.mjs).
// Path from repo root so CI (k6 run from root) loads the file written in the same job.
const GRANT_POOL_PATH = __ENV.CANOPY_PERF_GRANT_POOL_PATH || "perf/k6/canopy-api/data/grant-pool.json";
const grantPool = new SharedArray("grant-pool", function () {
  const data = open(GRANT_POOL_PATH);
  return [JSON.parse(data)]; // single element so grantPool[0] is { signer, grants }
});
...
export function setup() {
  ...
  if (grants.length === 0) {
    throw new Error(
      "grant-pool.json has no grants; run perf/scripts/generate-grant-pool.mjs first",
    );
  }
