...
// Load pre-created grant pool (generated by workflow or pnpm --filter @canopy/perf run generate-grant-pool).
// Path relative to script (bundle in dist/), so ../data = perf/k6/canopy-api/data/.
const grantPool = new SharedArray("grant-pool", function () {
  const data = open("../data/grant-pool.json");
...
export function setup() {
  ...
  if (grants.length === 0) {
    throw new Error(
      "grant-pool.json has no grants; run pnpm --filter @canopy/perf run generate-grant-pool first",
    );
  }
