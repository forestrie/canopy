...
- **Environment and config.** Env vars in use (canopy-api): `ROOT_LOG_ID`, `DELEGATION_SIGNER_URL`, `DELEGATION_SIGNER_BEARER_TOKEN`, `DELEGATION_SIGNER_PUBLIC_KEY_TOKEN`, `UNIVOCITY_SERVICE_URL`, `SEQUENCING_QUEUE`, `QUEUE_SHARD_COUNT`, `R2_GRANTS`, `MASSIF_HEIGHT`, `SUPPORTED_CHAINS_RPC`, etc. Custodian adds `CUSTODIAN_URL`, `CUSTODIAN_BOOTSTRAP_APP_TOKEN`. Subplan 06 may add settlement-related config (queue, DO, or callback URL). Per-chain RPC and supported chain ids: `SUPPORTED_CHAINS_RPC` (see ADR-0010); per-forest Univocity contract address comes from genesis chain binding, not deployment env.
