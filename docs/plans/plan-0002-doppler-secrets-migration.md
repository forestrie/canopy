...
| `SCRAPI_API_KEY`       | 2025-12-29               | Performance tests: auth for queue stats and API in `perf-canopy.yml` and taskfiles `perf.yml`.                                                                                                                                             |
...
- **Perf env files**: `perf/.env.dev` and `perf/.env.prod` are committed and contain non-secret config (URLs, log IDs); perf workflow also sources these and uses `SCRAPI_API_KEY` from GitHub Secrets. Doppler could eventually supply perf-related secrets only; config can stay in repo or move per team choice.
...
| **.github/workflows/perf-canopy.yml**          | `secrets.SCRAPI_API_KEY`, `secrets.CANOPY_X402_DEV_PRIVATE_KEY`                                         | From GitHub (synced from **canopy-ci**). No change.                                                                                                                                                                                                                                                                                                  |
...
| **taskfiles/perf.yml**                                                       | `SCRAPI_API_KEY`.                                                | Same.                                                                                                 |
...
- **Keys to include**: Everything workflows or Taskfiles in CI use **except** those that are only consumed by Cloudflare:
  - `CLOUDFLARE_API_TOKEN`, `R2_ADMIN`, `SCRAPI_API_KEY`, `CANOPY_X402_DEV_PRIVATE_KEY`, `GITAPP_PRIVATE_KEY`, `GITAPP_ID`; optionally `ANTHROPIC_API_KEY` and others from `.env.example.secrets` that CI needs.
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------- |
| `CLOUDFLARE_API_TOKEN`, `R2_ADMIN`, `SCRAPI_API_KEY`, `CANOPY_X402_DEV_PRIVATE_KEY`, `GITAPP_ID`, `GITAPP_PRIVATE_KEY`, optionally `ANTHROPIC_API_KEY`. | `CDP_API_KEY_ID`, `CDP_API_KEY_SECRET`; optionally other Worker-only secrets. | Any key you choose not to put in Doppler. |
...
1. Create Doppler project **canopy-ci** and a config (e.g. `github` or `ci`). Add secrets/variables needed by GitHub Actions only: `CLOUDFLARE_API_TOKEN`, `R2_ADMIN`, `SCRAPI_API_KEY`, `CANOPY_X402_DEV_PRIVATE_KEY`, `GITAPP_PRIVATE_KEY`, `GITAPP_ID`, and optionally `ANTHROPIC_API_KEY`. Set unmasked for any that should sync as GitHub variables (e.g. `GITAPP_ID`). Do not add `CDP_API_KEY_ID` or `CDP_API_KEY_SECRET`.
