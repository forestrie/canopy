...
| **6.5**  | **grant-shared.yml**                       | Add `taskfiles/grant-shared.yml`: vars (CANOPY*BASE_URL, SCRAPI_API_KEY, GRANT_DATA_DIR, MASSIF_HEIGHT, POLL*\*); tasks poll-status, resolve-receipt invoking scripts.                                   | `task grant-shared:poll-status` and resolve-receipt run with required vars.                                                                 |
