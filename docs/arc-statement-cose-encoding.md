...
| ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| `perf/scripts/generate-grant-pool.ts` (@canopy/perf)       | Creates grants via API (shared `encodeGrantRequest`); writes **grant-pool.json** with `signer` (hex) and `grants[]`. Same signer used as kid in k6 COSE. | **Register-grant**         |
